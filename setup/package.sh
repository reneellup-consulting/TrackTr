...
check_requirement "Tracktr server archive" "ls ../target/tracker-server.jar" "Missing tracktr archive"
...
prepare () {
  mkdir -p out/{conf,data,lib,logs,legacy,web,schema,templates}

  cp ../target/tracker-server.jar out
  cp ../target/lib/* out/lib
  cp ../schema/* out/schema
  cp -r ../templates/* out/templates
  cp -r ../tracktr-web/build/* out/web
  cp default.xml out/conf
  cp tracktr.xml out/conf
...
package_other () {
  ...
  cp README.txt out
  cd out
  zip -q -r ../tracktr-other-$VERSION.zip *
  cd ..
  rm out/README.txt
...
package_windows () {
  ...
  rm -rf jdk-*
  wine app/ISCC.exe tracktr.iss >/dev/null
  rm -rf out/jre
  zip -q -j tracktr-windows-64-$VERSION.zip Output/tracktr-setup.exe README.txt
  rm -r Output
...
package_linux () {
  chmod +x setup.sh
  cp setup.sh out
  cp tracktr.service out

  tar -xf OpenJDK*$1_linux*.tar.gz
  jlink --module-path jdk-*/jmods --add-modules java.se,jdk.charsets,jdk.crypto.ec --output out/jre
  rm -rf jdk-*
  makeself --needroot --quiet --notemp out tracktr.run "tracktr" ./setup.sh
  rm -rf out/jre

  zip -q -j tracktr-linux-$2-$VERSION.zip tracktr.run README.txt

  rm tracktr.run
  rm out/setup.sh
  rm out/tracktr.service
}
